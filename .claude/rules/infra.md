...
- DNS: the project domain (`kuutti.app`, variable `domain`) is registered through Route 53 in this account; the bootstrap adopts the zone the registration created and declares the domain-wide records (CAA, SPF, DMARC, null MX; `bootstrap/dns.tf`), and each environment declares its own records in `envs/<env>/dns.tf`. No records in the console (the three Amplify Hosting wrote for the placeholder site are the documented exception, ADR-001).
