...
Source maps: the `@sentry/react-native/expo` plugin in `app.json` names the EU server, the organisation and project `mobile`, and uploads maps during EAS Build when `SENTRY_AUTH_TOKEN` is an EAS secret; for updates, `deploy.yml` runs `sentry-expo-upload-sourcemaps dist` with the token from the GitHub environment and names the organisation in `SENTRY_ORG` (the uploader reads the environment before `app.json`). The organisation slug is `kuutti-fi`; `app.json` still says `kuutti` until the next native change, so the `preview` profile in `eas.json` skips the build-time upload meanwhile. The token is never in the repository or at repository level.
