...
- `pnpm env:doctor` checks Node, pnpm, Docker, `.env` and ports and prints the fix for each failure (`pnpm doctor` is pnpm's own). `docker compose up -d --wait` alone starts Postgres, the S3 stand-in (versitygw, http://127.0.0.1:9000) and the mock bank IdP (http://127.0.0.1:8080/ftn); `env:up` does that for you. `node services/mock-idp/verify.ts` runs a bank login against the mock.
