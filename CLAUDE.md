...
- One at a time: `pnpm dev` runs the API with tsx watch on port 3000. `pnpm dev:mobile` starts the Expo dev client; never Expo Go. It and `env:up` sign Metro's manifest with `apps/mobile/keys/private-key.pem`, which the dev client requires (ADR-004). `pnpm dev:admin` starts Vite.
