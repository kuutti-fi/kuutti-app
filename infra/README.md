...
6. **GitHub.** The environment `preview` with the maintainer as required reviewer (Trust, above: every preview deployment is a click, and ADR-004 depends on it) and no deployment branch restriction (pull requests deploy from any branch of this repository), four variables and two secrets, then the switch:
